        ...
        getById: async (req, res) => {
            try {
                ...
                if (!order) {
                    return res
                        .status(404)
                        .json({ message: "Sale order not found." });
                }

                const staffRecord = await getStaffRecord(
                    userId,
                    order.BusinessID,
                );
                if (!staffRecord) {
        ...
        create: async (req, res) => {
            try {
                ...
                for (const item of items) {
                    ...
                    const stock = await getInventoryItem(
                        warehouseId,
                        productId,
                    );
                ...
                await logAudit(pool, {
                    ...
                    entityId: soId,
                    details: `Created sale order for "${customerName}" from warehouse "${warehouse.WarehouseName}" with ${items.length} item(s).`,
                });
                return res
                    .status(201)
                    .json({
            ...
            try {
                ...
                if (status === "Cancelled") {
                    ...
                    await logAudit(pool, {
                        ...
                        entityType: "SaleOrder",
                        entityId: id,
                        details: `Cancelled sale order #${id}.`,
                    });
                    return res
                        .status(200)
                        .json({ message: "Sale order cancelled." });
                }
                ...
                if (items.length === 0) {
                    return res
                        ...
                        .json({
                            message:
                                "Cannot fulfill a sale order with no items.",
                        });
                }
                ...
                await logAudit(pool, {
                    ...
                    entityType: "SaleOrder",
                    entityId: id,
                    details: `Fulfilled sale order #${id} for "${order.CustomerName}" from warehouse #${order.WarehouseID}. Inventory decremented for ${items.length} item(s).`,
                });
                return res
                    .status(200)
                    .json({
                        message: "Sale order fulfilled. Inventory updated.",
                    });
            } catch (error) {
            ...
            try {
                ...
                if (!product || product.BusinessID !== order.BusinessID) {
                    return res
                        .status(400)
                        .json({
                            message: "Invalid product for this business.",
                        });
                }
            ...
            try {
                ...
                if (!item || item.SOID !== Number(id)) {
                    return res
                        .status(404)
                        .json({
                            message: "Item not found in this sale order.",
                        });
                }
