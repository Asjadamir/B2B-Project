...
const purchaseOrderControllers = {
    getSupplierProducts: asyncHandler(async (req, res) => {
        ...
        const products = await queries.getProductsBySupplier(
            supplierId,
            businessId,
        );
    ...
    create: asyncHandler(async (req, res) => {
        ...
        if (!businessId || !supplierId || !warehouseId) {
            return res
                ...
                .json({
                    message:
                        "businessId, supplierId, and warehouseId are required.",
                });
        }
        ...
        if (!PO_ROLES.includes(staffRecord.RoleName)) {
            return res
                ...
                .json({
                    message:
                        "Only Owner, Manager, or Procurement Officer can create purchase orders.",
                });
        }
        ...
        for (const item of items) {
            ...
            if (!productId || !quantity || unitCost == null) {
                return res
                    ...
                    .json({
                        message:
                            "Each item requires productId, quantity, and unitCost.",
                    });
            }
            if (quantity <= 0) {
                return res
            ...
            if (unitCost < 0) {
                return res
                    .status(400)
                    .json({ message: "Item unit cost cannot be negative." });
            }
            ...
            const linked = await queries.isProductLinkedToSupplier(
                productId,
                supplierId,
            );
            if (!linked) {
                return res
                    .status(400)
                    .json({
                        message: `Product "${product.ProductName}" is not linked to the selected supplier.`,
                    });
            }
        }
        ...
        return res
            .status(201)
            .json({ message: "Purchase order created successfully.", poId });
    }),
    ...
    updateStatus: asyncHandler(async (req, res) => {
        ...
        if (!["Received", "Cancelled"].includes(status)) {
            return res
                .status(400)
                .json({ message: "status must be 'Received' or 'Cancelled'." });
        }
        ...
        if (order.Status !== "Pending") {
            return res
                .status(409)
                .json({
                    message: `Cannot update a ${order.Status} purchase order.`,
                });
        }
        ...
        if (status === "Cancelled") {
            if (!CANCEL_ROLES.includes(staffRecord.RoleName)) {
                return res
                    ...
                    .json({
                        message:
                            "Only Owner or Manager can cancel purchase orders.",
                    });
            }
            const request = new sql.Request();
            request.input("ActorID", sql.Int, userId);
            ...
            return res
                .status(200)
                .json({ message: "Purchase order cancelled." });
        }
        ...
        if (!PO_ROLES.includes(staffRecord.RoleName)) {
            return res
                ...
                .json({
                    message:
                        "Only Owner, Manager, or Procurement Officer can mark orders as received.",
                });
        }
        ...
        return res
            ...
            .json({
                message:
                    "Purchase order marked as received. Inventory updated.",
            });
    }),
    ...
    addItem: asyncHandler(async (req, res) => {
        ...
        if (!productId || !quantity || unitCost == null) {
            return res
                .status(400)
                .json({
                    message: "productId, quantity, and unitCost are required.",
                });
        }
        if (quantity <= 0) {
            return res
        ...
        if (unitCost < 0) {
            return res
                .status(400)
                .json({ message: "Unit cost cannot be negative." });
        }
        ...
        if (order.Status !== "Pending") {
            return res
                .status(409)
                .json({
                    message: `Cannot modify a ${order.Status} purchase order.`,
                });
        }
        ...
        if (!PO_ROLES.includes(staffRecord.RoleName)) {
            return res
                ...
                .json({
                    message:
                        "Only Owner, Manager, or Procurement Officer can modify purchase orders.",
                });
        }
        ...
        if (!linked) {
            return res
                .status(400)
                .json({
                    message: `Product "${product.ProductName}" is not linked to this PO's supplier.`,
                });
        }
        ...
        if (duplicate) {
            return res
                ...
                .json({
                    message:
                        "This product is already in the purchase order. Edit the existing line item instead.",
                });
        }

        const itemId = await queries.addPOItem(
            id,
            productId,
            quantity,
            unitCost,
        ...
        await logAudit({
            ...
            entityId: id,
            details: `Added ${quantity} x "${product.ProductName}" at $${unitCost} to purchase order #${id}.`,
        });
        return res
            .status(201)
            .json({ message: "Item added to purchase order.", itemId });
    }),
    ...
    updateItem: asyncHandler(async (req, res) => {
        ...
        if (!quantity || unitCost == null) {
            return res
                .status(400)
                .json({ message: "quantity and unitCost are required." });
        }
        if (quantity <= 0) {
            return res
                .status(400)
        ...
        if (!item || item.POID !== Number(id)) {
            return res
                .status(404)
                .json({ message: "Item not found in this purchase order." });
        }
        ...
        await logAudit({
            ...
            entityType: "PurchaseOrder",
            entityId: id,
            details: `Updated item #${itemId} in purchase order #${id}: quantity ${quantity}, unit cost $${unitCost}.`,
        });
    ...
    removeItem: asyncHandler(async (req, res) => {
        ...
        await logAudit({
            ...
            entityType: "PurchaseOrder",
            entityId: id,
            details: `Removed item #${itemId} from purchase order #${id}.`,
        });
        return res
            .status(200)
            .json({ message: "Item removed from purchase order." });
    }),
};
