...
import { fetchSaleOrderById, updateSaleOrderStatusThunk, clearCurrentSaleOrder } from "@/store/saleOrderSlice";
...
import { ArrowLeft, CheckCircle, XCircle, User, Phone, MapPin } from "lucide-react";
...
export default function SaleOrderDetailPage() {
    ...
    async function handleStatusUpdate(status) {
        const r = await dispatch(updateSaleOrderStatusThunk({ id: soId, status }));
        if (!r.error) { setConfirmAction(null); toast.success(`Order marked as ${status}`); dispatch(fetchSaleOrderById(soId)); }
        ...
        <div className="p-6 max-w-3xl mx-auto space-y-6">
            <div className="flex items-center gap-3">
                ...
                <div className="flex-1 flex items-center justify-between">
                    <div>
                        <h1 className="text-xl font-bold">SO-{String(order.SOID).padStart(4, "0")}</h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1.5">
                            ...
                            {order.CustomerContact && (
                                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                    <Phone className="size-3.5" /> {order.CustomerContact}
                                </div>
                            )}
                ...
                <Card>
                    <CardContent className="p-4 space-y-2">
                        <p className="text-xs text-muted-foreground uppercase tracking-wider">Order Info</p>
                            ...
                            {items.length === 0 ? (
                                ...
                                items.map((item, i) => (
                                    <TableRow key={i}>
                                        ...
                                        <TableCell className="text-right font-medium">${(Number(item.Quantity) * Number(item.UnitPrice)).toFixed(2)}</TableCell>
                                    </TableRow>
                                ))
                            )}
                        </TableBody>
            ...
            {order.Status === "Pending" && (
                <div className="flex gap-3 justify-end">
                    ...
                    <Button className="gap-2 bg-green-600 hover:bg-green-700 text-white" onClick={() => setConfirmAction("Fulfilled")}>
            ...
            <Dialog open={!!confirmAction} onOpenChange={(v) => !v && setConfirmAction(null)}>
                <DialogContent className="sm:max-w-sm">
                    <DialogHeader>
                        <DialogTitle>{confirmAction === "Fulfilled" ? "Fulfill Order" : "Cancel Order"}</DialogTitle>
                        <DialogDescription>
                    ...
                    <DialogFooter>
                        ...
                        <Button
                            ...
                            className={confirmAction === "Fulfilled" ? "bg-green-600 hover:bg-green-700" : ""}
