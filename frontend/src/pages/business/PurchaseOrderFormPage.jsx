...
import { z } from "zod";
import { createPurchaseOrderThunk } from "@/store/purchaseOrderSlice";
import { fetchSuppliers } from "@/store/supplierSlice";
import { fetchWarehouses } from "@/store/warehouseSlice";
import { getSupplierProducts } from "@/lib/api";
...
const itemSchema = z.object({
    ...
    unitCost: z.string().min(1).refine((v) => Number(v) >= 0, "Cannot be negative"),
});
        ...
        <div className="p-6 max-w-3xl mx-auto space-y-6">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Create a purchase order from a supplier</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                                ...
                                <Select value={watch("supplierId")} onValueChange={(v) => { setValue("supplierId", v); setValue("items", [{ productId: "", quantity: "1", unitCost: "" }]); }}>
                ...
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between pb-3">
                        ...
                        <Button type="button" variant="outline" size="sm" onClick={() => append({ productId: "", quantity: "1", unitCost: "" })}>
                    ...
                    <CardContent className="space-y-3">
                        ...
                        {!watchedSupplierId && (
                            <p className="text-sm text-muted-foreground py-4 text-center">Select a supplier first to load available products.</p>
                        )}
                        {fields.map((field, index) => (
                            <div key={field.id} className="grid grid-cols-[1fr_100px_110px_36px] gap-2 items-start">
                                ...
                                <div className="space-y-1">
                                    <Input type="number" min="0" step="0.01" placeholder="Unit Cost" {...register(`items.${index}.unitCost`)} className={errors.items?.[index]?.unitCost ? "border-destructive" : ""} />
                                    {errors.items?.[index]?.unitCost && <p className="text-xs text-destructive">Required</p>}
                                </div>
                ...
                <div className="flex justify-end gap-3">
                    <Button type="button" variant="outline" onClick={() => navigate(`/business/${id}/purchase-orders`)}>Cancel</Button>
