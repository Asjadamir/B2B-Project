...
import { fetchSaleOrders, updateSaleOrderStatusThunk } from "@/store/saleOrderSlice";
...
export default function SaleOrdersPage() {
    ...
    useEffect(() => { dispatch(fetchSaleOrders(id)); }, [id, dispatch]);

    async function handleStatusUpdate(orderId, status) {
        const r = await dispatch(updateSaleOrderStatusThunk({ id: orderId, status }));
        if (!r.error) { setConfirmAction(null); toast.success(`Order marked as ${status}`); dispatch(fetchSaleOrders(id)); }
    ...
    return (
        <div className="p-6 space-y-5 max-w-5xl mx-auto">
            <div className="flex items-center justify-between gap-4">
                ...
                <Button onClick={() => navigate(`/business/${id}/sale-orders/new`)}>
                ...
                <Table>
                    ...
                    <TableBody>
                        {loading ? (
                                ...
                                <TableCell colSpan={7} className="text-center py-16">
                                    <div className="flex flex-col items-center gap-3">
                                        ...
                                        <p className="text-muted-foreground text-sm">
                                            {orders.length === 0 ? "No sale orders yet." : "No orders match the filter."}
                                        </p>
                                        {orders.length === 0 && (
                                            <Button size="sm" onClick={() => navigate(`/business/${id}/sale-orders/new`)}>
                            ...
                            filtered.map((o) => (
                                <TableRow key={o.SOID} className="cursor-pointer hover:bg-muted/50" onClick={() => navigate(`/business/${id}/sale-orders/${o.SOID}`)}>
                                    <TableCell className="font-mono text-sm font-medium">SO-{String(o.SOID).padStart(4, "0")}</TableCell>
                                    ...
                                    <TableCell onClick={(e) => e.stopPropagation()}>
                                        <div className="flex items-center gap-1">
                                            <Button variant="ghost" size="icon" className="size-7" onClick={() => navigate(`/business/${id}/sale-orders/${o.SOID}`)}>
                                                <Eye className="size-3.5" />
                                            </Button>
                                            {o.Status === "Pending" && (
                                                <>
                                                    <Button variant="ghost" size="icon" className="size-7 text-green-500 hover:text-green-600"
                                                        onClick={() => setConfirmAction({ id: o.SOID, status: "Fulfilled", label: `SO-${String(o.SOID).padStart(4, "0")}` })}>
                                                        <CheckCircle className="size-3.5" />
                                                    </Button>
                                                    <Button variant="ghost" size="icon" className="size-7 text-destructive hover:text-destructive"
                                                        onClick={() => setConfirmAction({ id: o.SOID, status: "Cancelled", label: `SO-${String(o.SOID).padStart(4, "0")}` })}>
            ...
            <Dialog open={!!confirmAction} onOpenChange={(v) => !v && setConfirmAction(null)}>
                <DialogContent className="sm:max-w-sm">
                    <DialogHeader>
                        <DialogTitle>{confirmAction?.status === "Fulfilled" ? "Mark as Fulfilled" : "Cancel Order"}</DialogTitle>
                        <DialogDescription>
                    ...
                    <DialogFooter>
                        ...
                        <Button
                            ...
                            className={confirmAction?.status === "Fulfilled" ? "bg-green-600 hover:bg-green-700" : ""}
