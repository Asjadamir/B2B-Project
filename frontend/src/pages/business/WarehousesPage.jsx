...
function WarehouseSheet({ open, onOpenChange, initial, onSubmit, loading }) {
    ...
    useEffect(() => {
        reset(initial || { warehouseName: "", address: "", city: "" });
    }, [initial, open, reset]);
                ...
                >
                    <div className="space-y-1.5">
                        ...
                        <Input
                            ...
                            className={
                                errors.warehouseName ? "border-destructive" : ""
                            }
                        />
...
export default function WarehousesPage() {
    ...
    async function handleCreate(form) {
        const r = await dispatch(
            createWarehouseThunk({ ...form, businessId: Number(id) }),
        );
        if (!r.error) {
            setSheetOpen(false);
            dispatch(fetchWarehouses(id));
    ...
    async function handleUpdate(form) {
        const r = await dispatch(
            updateWarehouseThunk({ id: editTarget.WarehouseID, data: form }),
        );
        if (!r.error) {
            setEditTarget(null);
            dispatch(fetchWarehouses(id));
    ...
    async function handleDelete() {
        const r = await dispatch(
            deleteWarehouseThunk(deleteTarget.WarehouseID),
        );
        if (!r.error) {
        ...
        <div className="p-6 space-y-5 max-w-4xl mx-auto">
            <div className="flex items-center justify-between gap-4">
                ...
                <Button onClick={() => setSheetOpen(true)}>
                    <Plus className="size-4 mr-1.5" /> New Warehouse
                </Button>
            </div>
                    ...
                    <CardContent className="py-16 flex flex-col items-center gap-4">
                        ...
                        <div className="text-center space-y-1">
                            ...
                            <p className="text-sm text-muted-foreground">
                                Add warehouse locations to manage inventory and
                                staff
                            </p>
                        </div>
                        <Button onClick={() => setSheetOpen(true)}>
                            <Plus className="size-4 mr-1.5" /> Add first
                            warehouse
                        </Button>
                    </CardContent>
                </Card>
                    ...
                    >
                        ...
                        <CardFooter className="pt-0 gap-1.5">
                            ...
                            <Button
                                variant="ghost"
                                size="sm"
                                className="h-7 px-2 text-xs ml-auto text-primary"
                                onClick={() =>
                                    navigate(
                                        `/business/${id}/warehouses/${w.WarehouseID}`,
                                    )
                                }
                            >
                                Manage Staff{" "}
